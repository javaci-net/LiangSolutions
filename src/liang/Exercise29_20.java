...
public class Exercise29_20 {
  ...
  public static class WeightedGraph<V> extends UnweightedGraph<V> {
    ...
    public MST getMinimumSpanningTree(int startingVertex) {
      ...
      boolean[] isInT = new boolean[vertices.size()]; // Inserted
      ...
      while (T.size() < getSize()) {
        ...
        for (int i = 0; i < getSize(); i++) {
          if (!isInT[i] && cost[i] < currentMinCost) { // Replace T.contains(i) by isInT[i]
        ...
        isInT[u] = true; // Inserted
        
        // Adjust cost[v] for v that is adjacent to u and v in V - T
        for (Edge e : neighbors.get(u)) {
          if (!isInT[e.v] && cost[e.v] > ((WeightedEdge)e).weight) { // Replace T.contains(e.v) by isInT[e.v]
